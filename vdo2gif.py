...
argparser.add_argument('-f','--fps',type=int, action='store',dest='framerate',default=25, help='framerate (less frame, less size of gif). default:25')
...
if args.time != None:
    ...
    if len(startTime) > 3 or len(endTime) > 3:
        ...
        sys.exit()
    clip = (VideoFileClip(args.inputdir).subclip((float(startTime[0]),float(startTime[1]),float(startTime[2])),(float(endTime[0]),float(endTime[1]),float(endTime[2]))).resize(0.8))
